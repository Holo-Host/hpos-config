...
impl std::fmt::Display for HoloPortConfiguration {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "name: {:?}, email: {}, admin_pubkey: {}, seed_key: {:?}, seed: {}, seed_sig: {}",
            &self.name, &self.email, &self.admin_pubkey, &self.seed_key, &self.seed, &self.seed_sig,
        )
    }
}
        ...
        *array_mut_ref![seed_cipher, AEAD_TAGBYTES, HOLO_ENTROPY_SIZE] = *array_ref![seed_encrypted, 0, HOLO_ENTROPY_SIZE];
        ...
        )? != seed_entropy
        {
            return Err(ConfigurationError::Generic(format!(
                "Unable to recover seed entropy of config: {:?}",
                config
            )));
        };
        Ok(config)
...
pub fn encrypt_seed_with(seed: &[u8], seed_key: &[u8]) -> Result<Vec<u8>, ConfigurationError> {
    let mut seed_enc =
        aes::ecb_encryptor(aes::KeySize::KeySize256, &seed_key, blockmodes::NoPadding);
...
pub fn blind_key_from(admin_key: &[u8; 32]) -> Result<Keypair, ConfigurationError> {
...
pub fn sign_with_prefix(
    ...
    base64::encode(
        &keypair
            .sign(&format!("[{}, {}]", prefix, payload).as_bytes())
            .to_bytes()
            .into_iter(),
    )
}
...
pub fn authenticate_monotonically(
    ...
    Ok(sig == sign_with_prefix(payload, prefix - 0, keypair)
        || (prefix > 0 && sig == sign_with_prefix(payload, prefix - 1, keypair)))
}
