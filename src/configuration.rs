...
pub fn sign_with_modulo(
    ...
    base64::encode(
        &keypair.sign(
            &format!("[{}, {}]", modulo, payload).as_bytes()
        ).to_bytes().into_iter()
    )
}
...
pub fn authenticate_monotonically(
    ...
    let modulo: u64 = seconds % delta.as_secs();
    Ok(   sign_with_modulo(payload, modulo - 0, keypair) == sig
       || sign_with_modulo(payload, modulo - 1, keypair) == sig)
}
